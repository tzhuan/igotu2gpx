...
/** Check to see if an option is a valid long option.  Called by nextOption().
 * Put in a separate method because this needs to be done twice.  (The C getOpt
            ...
            if (pfound->argumentType != LongOption::NoArgument) {
                ...
                if (optErr) {
                    ...
                    if (argv[optInd - 1].startsWith(QLatin1String("--")))
                        std::cerr
                            << qPrintable(OptionParser::tr("%1: option '--%2'"
                                     " doesn't allow an argument")
                                    .arg(argv.value(0), pfound->name))
                            << std::endl;
                    else
                        // +option or -option
                        std::cerr
                            << qPrintable(OptionParser::tr("%1: option '%2%3'"
                                     " doesn't allow an argument")
                ...
                if (optErr)
                    std::cerr
                        << qPrintable(OptionParser::tr("%1: option '%2'"
                                ...
                                .arg(argv.value(0), argv[optInd - 1]))
                        << std::endl;
...
QChar OptionParser::nextOption()
{
    ...
    if (!d->longOptions.isEmpty() &&
            (d->argv[d->optInd].startsWith(QLatin1String("--"))
    ...
    if ((temp[0] == QLatin1Char('W')) &&
        ...
        // No further cars in this d->argv element and no more d->argv elements
            ...
            if (d->optErr)
                // 1003.2 specifies the format of this message.
                std::cerr
                    << qPrintable(OptionParser::tr("%1: option requires an "
                                "argument -- %2")
                            .arg(d->argv.value(0), c))
                    << std::endl;
                ...
                if (d->posixlyCorrect && d->optArg == QLatin1String("--")) {
                    ...
                    if (d->optInd == unsigned(d->argv.size())) {
                        if (d->optErr)
                            // 1003.2 specifies the format of this message
                            std::cerr
                                << qPrintable(OptionParser::tr("%1: option "
