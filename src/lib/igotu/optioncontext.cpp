...
OptionContext::OptionContext
    (const QStringList &args, const QString &description,
...
QStringList OptionContext::parse() const
{
    ...
    if (helpEnabled) {
        longOpts += LongOption(QLatin1String("help"),
                LongOption::NoArgument, NULL, QLatin1Char('h'));
        longOpts += LongOption(QLatin1String("help-all"),
                LongOption::NoArgument, NULL, 2);
    ...
    while ((c = getopt.nextOption ()) != -1) {
        Q_ASSERT(c != 1);
        if (c == QLatin1Char(':'))
            throw Exception(tr("%1: option '%2' requires an argument")
                    .arg(args.value(0), QLatin1String("--") +
                        shortMap[getopt.invalidOptionValue()].longName));
        if (c == QLatin1Char('?'))
            throw Exception(tr("%1: unrecognized option '%2'")
        ...
        if (helpMap.contains(longOptIndex))
            printSpecialHelp(helpMap[longOptIndex]);
        OptionEntry entry = longOptIndex < 0 ?
            shortMap[c] : entryMap[longOptIndex];
        if (entry.argumentType == OptionEntry::NoArgument)
            entry.value->setValue
                (!entry.flags.testFlag(OptionEntry::ReverseOption));
        else
...
void OptionContext::printAllHelp() const
{
    printHelpUsage();
    printHelpHelp();
    QString result;
...
void OptionContext::printShortHelp() const
{
    printHelpUsage();
    printHelpHelp();
...
void OptionContext::printSpecialHelp(const OptionGroup &group) const
{
    printHelpUsage();
...
void OptionContext::printHelpUsage() const
{
    std::cout << qPrintable(tr("Usage:")) << std::endl;
    std::cout << "  " << qPrintable(args.value(0))
        ...
        unsigned leftIndent) const
{
    ...
    while (!descriptionText.isEmpty()) {
        if (newLine) {
            ...
            descriptionText = descriptionText.mid
                (indentExpression.matchedLength());
            linePart.setPattern(QString::fromLatin1("^[^\n]{0,%1}([\n\\s]|$)")
                    .arg(totalLength - indent));
            newLine = false;
        }
        if (!result.isEmpty())
            result += QString(indent, QLatin1Char(' '));
        if (linePart.indexIn(descriptionText) >= 0) {
            ...
            result += descriptionText.left(totalLength - indent);
            descriptionText = descriptionText.mid(totalLength - indent);
        }
        if (result.right(1) == QLatin1String("\n"))
            newLine = true;
        else
...
void OptionContext::printHelpEntry(const QString &longName,
        const QChar &shortName, const QString &description,
        const QString &argDescription) const
{
    QString result;
